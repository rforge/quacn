...
\usepackage{hyperref} 
\usepackage[left=1in,right=.75in,vmargin=1in,reversemp,marginpar=0.5in,marginparsep=0.2in]{geometry}
...
\def\calcdesc#1{\marginnote{\raggedleft\small\sffamily\color{gray} #1}\ignorespaces}
...
library("RBGL")
@ 
We generate a random unweighted undirected graph with 8 nodes.  This graph will be used to explain the implemented methods. To analyze a network the network has to be represented by a \textit{graphNEL}-object, which is part of the Bioconductor \texttt{graph} package.\\
...
\label{sec:desc}
This section provides a overview of the network descriptors that are included in the \texttt{QuACN} package. Here we describe the respective descriptor and how to call it in R.  The gray numbers in the left margin show the necessary descriptor numbers for the \texttt{calculateDescriptors} method; see section \ref{calculateDescriptors} below for details.
...
\calcdesc{\bfseries 1000}
\noindent This section describes network measures based on distances in the network.
...
\subsubsection*{Harary Index \cite{balaban_ivanciuc_2000}:}
\begin{equation}
\calcdesc{1002}
...
\label{sec:otherInv}
\calcdesc{\bfseries 2000}
\noindent This section describes network measures based on other invariants than distances.
...
\label{sec:clE}
\calcdesc{\bfseries 3000}
\noindent These measures are based on grouping the elements of an arbitrary graph invariant (vertices, edges, and distances etc.) using an equivalence criterion.
...
\calcdesc{3014}
I^E_D(G) &:=& -\sum_{i=0}^{\rho(G)} \frac{2 k_i}{N (N-1)} \log \frac{2 k_i}{N (N-1)}.
\end{eqnarray}
%
...
\begin{equation}
\calcdesc{3009}
I_{VC}^v(G):= \frac{1}{N} \sum_{i=1}^{N}v^c_i,
\end{equation}
...
\subsubsection*{Graph distance complexity \cite{raychaudhury_1984}:}
\begin{equation}
\calcdesc{3015}
I_{VC}(G) = \frac{1}{N} \sum_{i=1}^N I_{DC, \delta_D}(v_i),
\end{equation}
$I_{DC, \delta_D}$ being is the local invariant
\begin{equation}
I_{DC, \delta_D}(v_i) = -\sum_{j=1}^{\mathrm{ecc}(v_i)} a^i_j \cdot \frac{j}{\delta_D(v_i)} \log \frac{j}{\delta_D(v_i)},
\end{equation}
where $\mathrm{ecc}(v_i)$ denotes the eccentricity of the vertex $v_i$, $a^i_j$ is the number of vertices in its $j$-sphere, and $\delta_D(v_i)$ is its distance degree.
...
\subsubsection*{Mean Information Content of Distance Degrees \cite{skorobogatov}:}
\begin{equation}
\calcdesc{3016}
I_{\delta_D}(G) := -\sum_{i=1}^N \frac{\delta_D(v_i)}{2 W} \log \frac{\delta_D(v_i)}{2 W},
\end{equation}
where $\delta_D(v_i)$ is the distance degree of the $i$th vertex and $W$ is the Wiener index.
...
\subsubsection*{Mean Information Content of Distance-Degree Equality \cite{bonchev_2}:}
\begin{equation}
\calcdesc{3017}
I_{D, \delta_D}(G) = -\sum_{i=1}^k \frac{|N_i|}{N} \log \frac{|N_i|}{N},
\end{equation}
where $N_1, N_2, \ldots, N_k$ is the vertex partition of $N(G)$ such that each $N_i$ contains vertices having the same distance degree $\delta_D$.
...
\subsubsection*{Compactness Measure Based on Distance Degrees \cite{basak_1994}:}
\begin{equation}
\calcdesc{3018}
I_{C, \delta_D}(G) = 2 W \log(2 W) - \sum_k q_k \log q_k,
\end{equation}
where $W$ is the Wiener index and $q_k$ is the sum of the distance degrees of all vertices located at a topological distance of $k$ from the center of the graph.
...
\subsubsection*{Information Layer Index \cite{konstantinova_2006}:}
\begin{equation}
\calcdesc{3019}
I_{Sp} = -\sum_{i=1}^N \sum_{j=1}^{\mathrm{ecc}(v_i)} \frac{a^i_j}{N} \log \frac{a^i_j}{N},
\end{equation}
where $\mathrm{ecc}(v_i)$ denotes the eccentricity of the vertex $v_i$ and $a^i_j$ is the number of vertices in its $j$-sphere.
...
\calcdesc{\bfseries 4000}
\noindent This group contains miscellaneous descriptors which have been discovered quite recently.
...
\label{sec:dE}
\calcdesc{\bfseries 5000}
\noindent Measures of this group \cite{dehmer_functionals_2008,dehmer_varmuza_2009} assign a probability value to each vertex of the network using a so-called information functional $f$ which captures structural information of the network $G$.
...
When using the \texttt{calculateDescriptors} method, the first (odd) numbers listed in the margin will use exponential weighting while the second (even) numbers use linear.
...
\begin{itemize}
\item \(f^{V_1}(v_i)\): \(c_j^{l_v^{\mu}}\) is the weighting coefficient for vertices on the \(j\)-sphere of \(v_i\) whose label is \(l_v^{\mu}\).  It can be defined by passing as the \textit{coeffMatrix} parameter to \textit{infoTheoreticLabeledV1} a matrix whose columns represent the elements and whose rows the values of \(j\); the columns have to be named according to the chemical symbols.  The matrix chosen by default represents \(c_j^a = c_j - m_a / 238\), where \(m_a\) is the atomic mass of the atom \(a\).  Values for \(c_j\) can be selected using \textit{coeff} and \textit{custCoeff} as described above; the default is \texttt{"lin"}.  Again, the first listed descriptor numbers for \texttt{calculateDescriptors} use exponentially growing weighting coefficients whereas the second numbers use linear.
...
\calcdesc{\bfseries 6000}
\noindent This class contains eigenvalue-based Descriptors proposed in Dehmer et. al \cite{dehmer_hindawi_2010}. 
...
\begin{enumerate}
\item \calcdesc{6001, 6002} Adjacency matrix
<<>>=
eigenvalueBased(g, adjacencyMatrix,2)
@ 
\item \calcdesc{6003, 6004} Laplacian matrix
<<>>=
eigenvalueBased(g, laplaceMatrix,2)
@
\item \calcdesc{6005, 6006} Distance matrix
<<>>=
eigenvalueBased(g, distanceMatrix,2)
@
\item \calcdesc{6007, 6008} Distance path Matrix
<<>>=
eigenvalueBased(g,distancePathMatrix,2)
@
\item \calcdesc{6009, 6010} Augmented vertex degree matrix
<<>>=
eigenvalueBased(g, augmentedMatrix,2)
@
\item \calcdesc{6011, 6012} Extended adjacency matrix
<<>>=
eigenvalueBased(g, extendedAdjacencyMatrix,2)
@
\item \calcdesc{6013, 6014} Vertex Connectivity matrix 
<<>>=
eigenvalueBased(g, vertConnectMatrix,2) 
@
\item \calcdesc{6015, 6016} Random Walk Markov matrix 
<<>>=
eigenvalueBased(g, randomWalkMatrix,2)  
@
\item \calcdesc{6017, 6018} Weighted structure function matrix $IM_1$
<<>>=
eigenvalueBased(g, weightStrucFuncMatrix_lin,2) 
@
\item \calcdesc{6019, 6020} Weighted structure function matrix $IM_2$
<<>>=
...
When using \texttt{calculateDescriptors} with the numbers in the margin, note that the odd ones set $s = 1$ by default (but allows it to be changed through an optional parameter) while the even numbers fix $s = 2$.
...
\calcdesc{\bfseries 7000}
\noindent These descriptors are based on the number and properties of certain subgraphs that can be derived for a given network.
...
\calcdesc{\bfseries 8000}
\noindent This group contains descriptors which are computed by adding up weights along certain paths in a graph.
...
name or by number, the latter of which can be found in gray type in the
margin of this vignette.  Numbers divisible by 1000 will trigger the
calculation of all the descriptors from a group.  It is also possible to
name the resulting columns according to the names given in this
document.

The function documentation contains a detailed usage description and a
