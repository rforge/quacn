...
Note that some combinations of these settings can cause the descriptor to retrun \textit{NaN}. In that case you have to check for \textit{warnings}.
...
This class contains Eigenvalue-based Descriptors proposed in Dehmer et. al \cite{dehmer_degree_2010}. With this function it is possible to calculate 5 descriptors ($H_{M_s(G)}$, $S_{M_s(G)}$ , $IS_{M_s(G)}$, $P_{M_s(G)}$, $IP_{M_s(G)}$) for 10 different matrices:
...
For a detailed descripton of this classs see Dehmer et. al \cite{dehmer_degree_2010}.
<<>>=
