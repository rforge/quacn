...
The \texttt{huXuID} method requires the input graph to store the atomic numbers in the \texttt{"atom"} vertex data attribute and the conventional bond order in the \texttt{"bond"} edge data attribute.
