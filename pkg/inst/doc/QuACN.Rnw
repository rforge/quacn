...
Note, that \texttt{QuACN} also depends on the \texttt{Rmpfr} package. Therefore, following software needs to be installed to install the package correctly. To install the software follow the instruction on respective website.

\begin{itemize}
 \item GMP: \url{http://gmplib.org/}
 \item MPFR: \url{http://www.mpfr.org/}
\end{itemize}

For more information see the documentation of the \texttt{Rmpfr} package:

\begin{itemize}
 \item R-Forge: \url{http://rmpfr.r-forge.r-project.org/}
 \item CRAN: \url{http://cran.r-project.org/web/packages/Rmpfr/}\end{itemize}
