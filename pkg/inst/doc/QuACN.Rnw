...
The \texttt{calculateDescriptors} function allows you to calculate a set
of descriptors on a list of input graphs.  The result is returned as a
data frame.  You can specify the desired functions by name or by number.
It is also possible to name the columns according to the names given in
this document.
