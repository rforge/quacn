...
\begin{itemize}
\item \texttt{"atom"}: Atomic number of a graph vertex.
\item \texttt{"weight"}: Conventional bond order of an edge, i.e. 1 for single bonds, 2 for double bonds, 3 for triple bonds and 1.5 for aromatic bonds.
\end{itemize}
...
% TODO citation?
We integrated 4 different information functionals \cite{dehmer_functionals_2008}:
...
\end{equation}
where $ c_k > 0$, $1 \leq k \leq \rho(G)$ and $\alpha>0$. Note that \(f^{\Delta}\) is well-defined for \(\alpha > 0\).
\end{enumerate}
...
Note, that some combinations of these settings can cause the descriptor to return \textit{NaN}. In that case it is the user's responsibility to check for \textit{warnings}. Moreover, the functional using degree degree (\textit{infofunct=degree}) associations is using the \texttt{Rmpfr} package to calcualte very small numbers. Thus, it can happen that some results are from the R-class \textit{mpfr}. This has to be considered fors further analysis.
...
\subsubsection*{Randi\'{c} connectivity ID number:} % TODO citation: Randi'c, 1984b
...
\subsubsection*{MINCID:} % TODO citation: Ivanciuc and Balaban, 1996b
...
\subsubsection*{Prime ID number:} % TODO citation: Randi'c, 1986b
...
\subsubsection*{Conventional bond order ID number:} % TODO citation: Randi'c and Jurs, 1989
...
The conventional bond order must be set as the \texttt{"weight"} edge data attribute of the input graph.

<<>>=
bondOrderID(g)
@

\subsubsection*{Balaban ID number:} % TODO citation: Balaban, 1987
...
\subsubsection*{MINBID:} % TODO citation: Ivanciuc and Balaban, 1996b
...
\subsubsection*{Weighted ID number:} % TODO citation: Szymanski, M"uller et al., 1986b
...
\subsubsection*{Hu-Xu ID number:} % TODO citation: Hu and Xu, 1997
...
The \texttt{huXuID} method requires the input graph to store the atomic numbers in the \texttt{"atom"} vertex data attribute and the conventional bond order in the \texttt{"weight"} edge data attribute.
