...
The functional based on degree-degree associations (\texttt{infofunct="degree"}) will often result in values that cannot be represented with standard R data types.  This problem manifests itself in \textit{NaN} return values.  The issue can be avoided by specifying a floating-point precision value greater than 53, using the \textit{prec} parameter (\texttt{prec=128} is usually enough).  In this case, the \texttt{Rmpfr} package will be used and the resulting list will contain vectors of the class \textit{mpfr}.  For the most part, these behave just like the usual numeric vectors, except that all calculations will result in \textit{mpfr} vectors.  \texttt{as.double} can be used to convert such a vector back to the regular \textit{numeric} class once the result is in the representable range (between about \(10^{-380}\) and \(10^{380}\)).  For example:
